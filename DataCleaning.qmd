...
# join atp and wta dataset into one massive dataset for app
# fill in missing values for atp via research
...
```{r atp prize money conversion}
# convert prize money column in atp to USD for analysis

usd_conversion <- atp |>
  pivot_wider(names_from = TOURNAMENT,
              values_from = WINNER_PRIZE) |>
  mutate(Wimbledon = Wimbledon*1.26,
         `French Open` = `French Open`*1.08,
...
## GrandSlam Data
The columns ending in (.x) apply to Men's Singles
The columns ending in (.y) apply to Women's Singles
The columns that don't have either apply to both Men's and Women's Singles results
```{r}
# join atp and wta into one massive dataset for app
grandslam <- atp_usd |>
  left_join(wta_usd, by = "YEAR") |>
  select(-c(GENDER.x, TOURNAMENT.y, GENDER.y)) |>
